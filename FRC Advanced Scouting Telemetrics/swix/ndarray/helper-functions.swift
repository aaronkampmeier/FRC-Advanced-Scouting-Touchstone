...
// NORM
func norm(_ x: ndarray, ord:Double=2) -> Double{
    ...
    return -1.0}
func count_nonzero(_ x:ndarray)->Double{
    return sum(abs(x) > S2_THRESHOLD)
}

// modifying elements of the array
func clip(_ a:ndarray, a_min:Double, a_max:Double)->ndarray{
    // clip the matrix
    var y = a.copy()
...
func reverse(_ x:ndarray) -> ndarray{
    // reverse the array
    let y = x.copy()
    vDSP_vrvrsD(!y, 1.stride, y.n.length)
    return y
}
func delete(_ x:ndarray, idx:ndarray) -> ndarray{
...
func `repeat`(_ x: ndarray, N:Int, axis:Int=0) -> ndarray{
...
func sort(_ x:ndarray)->ndarray{
    // sort the array and return a new array
    let y = x.copy()
    y.sort()
    return y
}
func unique(_ x:ndarray)->ndarray{
    var y = sort(x)
...
func shuffle(_ x:ndarray)->ndarray{
...
// SETS
func intersection(_ x: ndarray, y:ndarray)->ndarray{
    return unique(x[argwhere(in1d(x, y: y))])
}
func union(_ x:ndarray, y:ndarray)->ndarray{
    return unique(concat(x, y: y))
}
func in1d(_ x: ndarray, y:ndarray)->ndarray{
    if (x.n > 0 && y.n > 0){
...
func concat(_ x:ndarray, y:ndarray)->ndarray{
...
func argwhere(_ idx: ndarray) -> ndarray{
...
// LOGICAL
func logical_and(_ x:ndarray, y:ndarray)->ndarray{
    return x * y
}
func logical_or(_ x:ndarray, y:ndarray)->ndarray{
    var i = x + y
    let j = argwhere(i > 0.5)
    i[j] <- 1.0
    return i
}
func logical_not(_ x:ndarray)->ndarray{
    return 1-x
}
func logical_xor(_ x:ndarray, y:ndarray)->ndarray{
    let i = x + y
    let j = (i < 1.5) && (i > 0.5)
    return j
}
...
// PRINTING
func println(_ x: ndarray, prefix:String="array([", postfix:String="])", newline:String="\n", format:String="%.3f", seperator:String=", ", printAllElements:Bool=false){
    // print the matrix
...
func print(_ x: ndarray, prefix:String="ndarray([", postfix:String="])", format:String="%.3f", printWholeMatrix:Bool=false){
