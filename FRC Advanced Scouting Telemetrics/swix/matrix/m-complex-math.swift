...
func dot(_ A: matrix, x: ndarray) -> ndarray{
    return A.dot(x)
}
func svd(_ x: matrix, compute_uv:Bool=true) -> (matrix, ndarray, matrix){
    let (m, n) = x.shape
...
func solve(_ A: matrix, b: ndarray) -> ndarray{
    let (m, n) = A.shape
