...
func apply_function(_ function: (ndarray)->ndarray, x: matrix)->matrix{
...
// AXIS
func sum(_ x: matrix, axis:Int = -1) -> ndarray{
...
func prod(_ x: matrix, axis:Int = -1) -> ndarray{
    assert(axis==0 || axis==1, "if you want to sum over the entire matrix, call `sum(x.flat)`.")
    let y = log(x)
    let z = sum(y, axis:axis)
    return exp(z)
}
func mean(_ x:matrix, axis:Int = -1) -> ndarray{
